...
from constants import baseDir, rawDir, cruiseDataRsyncScriptName, cruiseDataBackupRsyncScriptName, cruiseDataHourlyRsyncScriptName, scriptDir, templatesDir
...
if not os.path.isfile(scriptPath) or build_confirmation_menu("\nRsync script: " + scriptFN + ", already exists.  Rebuild it?", defaultResponse=False):
    ...
    scriptFN = cruiseDataHourlyRsyncScriptName.replace('<cruise_id>', cruise['cruise_id'])
    scriptPath = os.path.join(cruiseDir, "scripts", scriptFN)
